...
from pgu.gui import widget, surface, basic, events
...
class _button(widget.Widget):
    ...
    def event(self,e):
        if e.type == events.ENTER: self.repaint()
        ...
        elif e.type == events.FOCUS: self.repaint()
        elif e.type == events.BLUR: self.repaint()
            ...
            self.state = 1
            self.repaint()
        elif e.type == events.KEYUP:
            if self.state == 1:
                sub = pygame.event.Event(events.CLICK,{'pos':(0,0),'button':1})
