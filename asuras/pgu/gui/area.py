...
from pgu.gui import container, table
from pgu.gui import group
from pgu.gui import events
from pgu.gui import basic, button, slider
...
#             box.style.height = self.style.height - hs.rect.h
        ...
        if self.vscrollbar:
            self.vscrollbar = slider.VScrollBar(box.offset[1],0, 65535, 0,step=self.step)
            self.td(self.vscrollbar)
            self.vscrollbar.connect(events.CHANGE, self._vscrollbar_changed, None)
        ...
        if self.hscrollbar:
            self.hscrollbar = slider.HScrollBar(box.offset[0], 0,65535, 0,step=self.step)
            self.hscrollbar.connect(events.CHANGE, self._hscrollbar_changed, None)
            self.tr()
            self.td(self.hscrollbar)
...
class List(ScrollArea):
    ...
    def __init__(self, width, height, **params):
        ...
        g = group.Group()
        self.group = g
        g.connect(events.CHANGE,self._change,None)
    ...
    def clear(self):
        ...
        self.group.connect(events.CHANGE,self._change,None)
        self.table.clear()
