...
from pgu.gui import widget, surface, events
...
class Container(widget.Widget):
    ...
    def update(self,s):
        ...
        for w in self.topaint:
            ...
            # Normally in events.PGU when a widget changes state (eg normal to highlighted state) the new
...
#                    sub.blit(tmp, (0,0), subrect)
    ...
    def event(self,e):
        ...
        if self.mywindow and e.type == events.MOUSEBUTTONDOWN:
            w = self.mywindow
        ...
        if not self.mywindow:
            #### by Gal Koren
            ##
            ## if e.type == events.FOCUS:
            if e.type == events.FOCUS and not self.myfocus:
                #self.first()
                pass
            elif e.type == events.EXIT:
                if self.myhover: self.exit(self.myhover)
            elif e.type == events.BLUR:
        ...
        w = self.myfocus
        if w:
            if e.type == events.MOUSEBUTTONUP or e.type == events.MOUSEBUTTONDOWN:
                sub = pygame.event.Event(e.type,{
                    'button':e.button,
                    'pos':(e.pos[0]-w.rect.x,e.pos[1]-w.rect.y)})
            elif e.type == events.CLICK and self.myhover is w:
                sub = pygame.event.Event(e.type,{
                    'button':e.button,
                    'pos':(e.pos[0]-w.rect.x,e.pos[1]-w.rect.y)})
            elif e.type == events.MOUSEMOTION:
            ...
            #elif e.type == events.CLICK: #a dead click
            #    sub = None

            if (sub):
                used = w._event(sub)

        if not used and e.type is events.KEYDOWN:
            if e.key is events.K_TAB and self.myfocus:
                if (e.mod & events.KMOD_SHIFT) == 0:
                    next(self.myfocus)
                else:
                    self.myfocus.previous()
                    return True
            elif e.key == events.K_UP:
                self._move_focus(0,-1)
                return True
            elif e.key == events.K_RIGHT:
                self._move_focus(1,0)
                return True
            elif e.key == events.K_DOWN:
                self._move_focus(0,1)
                return True
            elif e.key == events.K_LEFT:
                self._move_focus(-1,0)
                return True
        return used
    ...
    def add(self,w,x,y):
        ...
        w.style.y = y
        w.container = self
        #events.NOTE: this might fix it, sort of...
...
#            return widget.Widget.focus(self)
        ...
        self.myfocus = w
        w._event(pygame.event.Event(events.FOCUS))
    ...
    def blur(self,w=None):
        ...
        if self.myfocus is w:
            ...
            self.myfocus = None
            w._event(pygame.event.Event(events.BLUR))

    def enter(self,w):
        if self.myhover: self.exit(self.myhover)
        self.myhover = w
        w._event(pygame.event.Event(events.ENTER))

    def exit(self,w):
        if self.myhover and self.myhover is w:
            self.myhover = None
            w._event(pygame.event.Event(events.EXIT))
...
#     def next(self,w):
#         if w not in self.widgets: return #events.HACK: maybe.  this happens in windows for some reason...
#
