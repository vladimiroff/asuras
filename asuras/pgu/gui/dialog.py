...
from pgu.gui import table, area, events
from pgu.gui import basic, input, button
...
class Dialog(table.Table):
    ...
    def __init__(self,title,main,**params):
        ...
        clos.connect(events.CLICK,self.close,None)
...
class FileDialog(Dialog):
    ...
    def __init__(self, title_txt="File Browser", button_txt="Okay", cls="dialog", path=None):
        ...
        self.list.connect(events.CHANGE, self._item_select_changed_, None)
        self.button_ok.connect(events.CLICK, self._button_okay_clicked_, None)
        self.body.tr()
