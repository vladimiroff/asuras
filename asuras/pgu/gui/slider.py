...
from pgu.gui import events
from pgu.gui import widget
from pgu.gui import table
from pgu.gui import basic
...
class _slider(widget.Widget):
    ...
    def paint(self,s):
        ...
        if self.orient == SLIDER_HORIZONTAL:
    ...
    def event(self,e):
        ...
        r = pygame.rect.Rect(0,0,self.style.width,self.style.height)
        adj = 0
        if e.type == events.ENTER: self.repaint()
        elif e.type == events.EXIT: self.repaint()
            ...
            if self.bar.collidepoint(e.pos):
                ...
                self.grab = None
            self.repaint()
        elif e.type == events.MOUSEBUTTONUP:
            #x,y,adj = e.pos[0],e.pos[1],1
            self.repaint()
        elif e.type == events.MOUSEMOTION:
        ...
        elif e.type is events.KEYDOWN:
            if self.orient == SLIDER_HORIZONTAL and e.key == events.K_LEFT:
                self.value -= self.step
                used = True
            elif self.orient == SLIDER_HORIZONTAL and e.key == events.K_RIGHT:
                self.value += self.step
                used = True
            elif self.orient == SLIDER_VERTICAL and e.key == events.K_UP:
                self.value -= self.step
                used = True
            elif self.orient == SLIDER_VERTICAL and e.key == events.K_DOWN:
                self.value += self.step
                used = True
    ...
    def __setattr__(self,k,v):
        if k == 'value':
            ...
            v = max(v,self.min)
            v = min(v,self.max)
        _v = self.__dict__.get(k,events.NOATTR)
        self.__dict__[k]=v
        if k == 'value' and _v != events.NOATTR and _v != v:
...
class VSlider(_slider):
    ...
    def __init__(self,value,min,max,size,step=1,**params):
        ...
        _slider.__init__(self,value,_events.SLIDER_events.VERTICAL,min,max,size,step,**params)
...
class HSlider(_slider):
    ...
    def __init__(self,value,min,max,size,step=1,**params):
        ...
        _slider.__init__(self,value,SLIDER_HORIZONTAL,min,max,size,step,**params)
...
class HScrollBar(table.Table):
    ...
    def __init__(self,value,min,max,size,step=1,**params):
        ...
        self.slider = _slider(value,SLIDER_HORIZONTAL,min,max,size,step=step,cls=self.cls+'.slider')
        self.minus = basic.Image(self.style.minus)
        self.minus.connect(events.MOUSEBUTTONDOWN,self._click,-1)
        self.slider.connect(events.CHANGE,self.send,events.CHANGE)

        self.minus2 = basic.Image(self.style.minus)
        self.minus2.connect(events.MOUSEBUTTONDOWN,self._click,-1)

        self.plus = basic.Image(self.style.plus)
        self.plus.connect(events.MOUSEBUTTONDOWN,self._click,1)
    ...
    def resize(self,width=None,height=None):
        ...
        #events.HACK: handle theme sizing properly
...
class VScrollBar(table.Table):
    ...
    def __init__(self,value,min,max,size,step=1,**params):
        ...
        self.slider = _slider(value,SLIDER_VERTICAL,min,max,size,step=step,cls=self.cls+'.slider')
        self.slider.connect(events.CHANGE,self.send,events.CHANGE)
