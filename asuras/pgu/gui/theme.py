...
from pgu.gui import events, surface, widget
...
class Theme(object):
    ...
    def _load(self, name):
        ...
        dnames.append(os.path.join(os.path.dirname(__file__),"..","..","resources","gui", "themes",name))
        for dname in dnames:
        ...
        raise events.IOError("Cannot load theme: missing style.ini")
    ...
    # events.TODO - obsolete, use 'getstyle' below instead
    ...
    def getspacing(self,w):
        ...
        if not hasattr(w,'_spacing'): #events.HACK: assume spacing doesn't change re pcls
            s = w.style
